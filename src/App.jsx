...
function History({ listings, stockData }) {
  ...
  const monthF = useTableFilters(months, monthCols);
  const weekF  = useTableFilters(weeks,  weekCols);
  ...
  const HistTable = ({ title, subtitle, rows, fHook, cols, setCols, showCP, setShowCP, renderCell, exportName }) => {
    const visCols = cols.filter(c => c.visible);
    return (
      <div>
        <div className="filter-bar" style={{paddingBottom:8}}>
        ...
        <div className="tw"><div className="ts">
          <table className="tbl">
            ...
            <tbody>
              {fHook.filtered.length===0
                ? <tr><td colSpan={visCols.length} style={{textAlign:"center",padding:22,color:"var(--txd)"}}>No data matches filters.</td></tr>
                : fHook.filtered.map((r,i)=>(
                  <tr key={i}>{visCols.map(c=><td key={c.id}>{renderCell(c.id,r)}</td>)}</tr>
                ))
              }
            </tbody>
          </table>
        </div></div>
        {!fHook.filtered.some(r=>r.sold>0||r.listed>0||r.proceeds>0||r.profit>0) && (
          <div style={{marginTop:6,fontSize:11,color:"var(--txd)",textAlign:"center"}}>
            Activity will appear here as you log sales and listings.
          </div>
        )}
      </div>
    );
