...
async function getAllSubs() {
  const rows = await supa("GET", "push_subscriptions?select=subscription");
  return Array.isArray(rows) ? rows.map(r => r.subscription) : [];
}

async function saveSub(subscription) {
  // Delete existing row for this endpoint, then insert fresh
  // This avoids all upsert header complexity
  const enc = encodeURIComponent(subscription.endpoint);
  await supa("DELETE", `push_subscriptions?endpoint=eq.${enc}`);
  await supa("POST", "push_subscriptions",
...
async function removeSub(endpoint) {
  await supa("DELETE", `push_subscriptions?endpoint=eq.${encodeURIComponent(endpoint)}`);
}
...
module.exports = async function handler(req, res) {
  ...
  if (action === "send" && payload) {
    ...
    try {
      ...
      const expired = [];
      results.forEach((r,i) => {
        if (r.status==="fulfilled" && [404,410].includes(r.value?.status)) expired.push(subs[i].endpoint);
      });
      await Promise.all(expired.map(ep => removeSub(ep).catch(()=>{})));
      const sent = results.filter((r,i) => r.status==="fulfilled" && ![404,410].includes(r.value?.status)).length;
      return res.status(200).json({ sent, expired:expired.length, failed:results.filter(r=>r.status==="rejected").length });
    } catch(e) { return res.status(500).json({ error:e.message }); }
  }
